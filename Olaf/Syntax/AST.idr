...
namespace Olaf
  ...
  namespace Prog
    public export
    data AST a = Decl a String Ty Bool (Expr.AST a) (Prog.AST a)
               | Main a (Expr.AST a)
  public export
  Prog : Type
  Prog = Prog.AST FileContext

  export
  Functor (Prog.AST) where
    map f (Decl x y t b z w) = Decl (f x) y t b (map f z) (map f w)
    map f (Main x y) = Main (f x) (map f y)
